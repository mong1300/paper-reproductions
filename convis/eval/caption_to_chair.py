...
def _patched_evaluate(self):
    """COCOEvalCap.evaluate 에서 SPICE 만 뺀 판본."""
    ...
    gts = {i: self.coco.imgToAnns[i] for i in self.params["image_id"]}
    res = {i: self.cocoRes.imgToAnns[i] for i in self.params["image_id"]}
...
for start_idx in tqdm(
    ...
    # SPICE 제외 (Apple Silicon 에서 spice-1.0.jar 실행 불가). CHAIR 계산에는 쓰이지 않는다.
    coco_eval.evaluate = _patched_evaluate.__get__(coco_eval)
