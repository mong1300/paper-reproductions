...
import nltk
import json
# from pattern.en import singularize   # CLiPS Pattern 은 Python 3.12 미지원 -> inflect 로 대체
...
def combine_coco_captions(annotation_path):
    ...
    # train2014 는 선택 사항으로 바꿈: 평가 대상이 val2014 이미지뿐이라
    # train 어노테이션은 결과에 기여하지 않는다 (원본은 둘 다 요구).
    train_path = "%s/captions_%s2014.json" % (annotation_path, "train")

    val_caps = json.load(open("%s/captions_%s2014.json" % (annotation_path, "val")))
    train_caps = json.load(open(train_path)) if os.path.exists(train_path) else {
        "info": val_caps["info"], "licenses": val_caps["licenses"],
        "images": [], "annotations": [],
    }
    all_caps = {
...
def combine_coco_instances(annotation_path):
    ...
    # train2014 는 선택 사항 (평가 대상이 val2014 뿐이라 결과에 기여하지 않음)
    ...
    train_instances = json.load(open(train_path)) if os.path.exists(train_path) else {
        "info": val_instances["info"], "licenses": val_instances["licenses"],
        "categories": val_instances["categories"], "images": [], "annotations": [],
    }
    all_instances = {
...
class CHAIR(object):
    ...
    def get_annotations_from_segments(self):
        ...
        for i, annotation in enumerate(segment_annotations):
            if i % 1000 == 0:   # 항목마다 출력하면 로그가 수십 MB 가 된다
                sys.stdout.write(
    ...
    def compute_chair(self, cap_file):
        ...
        for i, cap_eval in enumerate(caps):
            ...
            cap_dict["metrics"] = {
                ...
                # SPICE 제외: Apple Silicon 에서 spice-1.0.jar 실행 불가 (lmdbjni x86 전용,
                # Nashorn 제거). CHAIR 계산과는 무관한 값이다.
                "SPICE": cap_eval.get("SPICE"),
        ...
        output["overall_metrics"] = {
            ...
            "SPICE": self.metrics.get("SPICE"),   # SPICE 제외 (Apple Silicon 미지원)
...
def print_metrics(hallucination_cap_dict, quiet=False):
    ...
    # SPICE 는 Apple Silicon 에서 계산 불가 -> None 이면 0 으로 표기 (CHAIR 와 무관)
    metric_string = "%0.01f\t%0.01f\t%0.01f\t%0.01f\t%0.01f" % (
        (sentence_metrics["SPICE"] or 0) * 100,
